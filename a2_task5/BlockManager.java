...
public class BlockManager
{
	...
	public static void main(String[] argv)
	{
		try
		{
			...
			try {
				...
				System.out.println("Stack is initially empty: " + e.getMessage());
			}
			...
			try {
				...
				System.out.println("Final stack state: " + e.getMessage());
			} catch (InvalidStackAccessException e) {
				System.out.println("Invalid access during final stats: " + e.getMessage());
			}
	...
	static class AcquireBlock extends BaseThread
	{
		...
		public void run()
		{
			...
			{
				System.out.println("AcquireBlock thread [TID=" + this.iTID + "] - " + e.getMessage());
			}
			catch(Exception e)
			{
				reportException(e);
			...
			if (!s1.isLocked()) System.out.println("All threads finished PHASE 1!");
			...
			while (mutex.isLocked() || !turnTestAndSet()) {
				System.out.println("AcquireBlock thread [TID=" + this.iTID + "] - attempted to start PHASE 2 but waiting for its turn");
			};
	...
	static class ReleaseBlock extends BaseThread
	{
		...
		public void run()
		{
			...
			{
				System.out.println("ReleaseBlock thread [TID=" + this.iTID + "] - " + e.getMessage());
			} catch(Exception e)
			{
				reportException(e);
				System.exit(1);
			...
			try
			{
				for(int i = 0; i < siThreadSteps; i++)
				{
					...
					try {
						...
						{
							char stackChar = soStack.getAt(s);
							System.out.print((s == soStack.getITop() ? "(" : "[") + stackChar + (s == soStack.getITop() ? ")" : "]"));
						}
					} catch (InvalidStackAccessException e) {
						System.out.print("Error accessing stack: " + e.getMessage());
					}
